...
class GemmaDetox:
    SYSTEM_PROMPT = (
        ...
        "3) Keep the sentence natural and fluent, stylistically close to the original.\n"
    ...
    def _build_prompt(self, text: str) -> str:
        # Простой текстовый промпт вместо чат-шаблона
        return (
...
def build_regexes() -> List[re.Pattern]:
    ...
    patterns.append(
        re.compile(
            r"б\s*л\s*[еe*#]?\s*[яya@4]+\s*(?:т|ть|\*+|#+)",
            re.IGNORECASE,
        )
    )
    # Общие корни со звездочками или решетками (ху*, пи*д*, на*уй и так далее)
...
def cleanse_text(text: str) -> str:
    t = str(text)
    # Удаляем явные токены (без учета регистра)
...
def main():
    ...
    ap.add_argument(
        "--input_tsv",
        type=str,
        default="DETOX_TATAR/data/dev_inputs.tsv",
    )
    ap.add_argument(
        "--output_dir",
        type=str,
        default="DETOX_TATAR/outputs_wordrm_gemma",
    )
    ...
    ap.add_argument(
        "--gemma_id",
        type=str,
        default="google/gemma-2-9b-it",  # подставь сюда свой 14B, если он у тебя есть
    )
    ...
    # 3) Gemma detox only for masked items, keep new tokens only, single line
    gemma = GemmaDetox(args.gemma_id, device, fp16=args.fp16)
    outputs: List[str] = []
    bs = args.batch_size
    total_batches = (len(cleaned) + bs - 1) // bs
    for i in tqdm(
        range(0, len(cleaned), bs),
        total=total_batches,
        desc="Detox gate",
        unit="batch",
    ):
    ...
    # Save TSV and optional ZIP
    tsv_path = os.path.join(args.output_dir, "submission_wordrm_gemma.tsv")
    pd.DataFrame(
        {"ID": ids, "tat_toxic": texts, "tat_detox1": final_out}
    ).to_csv(tsv_path, sep="\t", index=False)
    logging.info("Saved TSV: %s", tsv_path)
    if args.make_zip:
        zip_path = os.path.join(args.output_dir, "submission_wordrm_gemma.zip")
